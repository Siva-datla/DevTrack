...
export const Sidebar = ({
  ...
  return (
    <>
      ...
      {/* Sidebar Container: 
          Outer container clips smoothly with transition-[width] 
      */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 bg-white dark:bg-[#0c121e] border-r border-slate-200 dark:border-slate-800/80 flex flex-col overflow-hidden transition-[width,transform] duration-200 ease-out ${
          isMobileOpen
            ? 'translate-x-0 w-64'
            : '-translate-x-full lg:translate-x-0'
        } ${isCollapsed ? 'lg:w-[72px]' : 'lg:w-64'}`}
      >
        {/* Inner Container: ALWAYS fixed at 256px (w-64) so child elements NEVER reflow or wrap */}
        <div className="w-64 h-full flex flex-col flex-shrink-0">
          ...
          <div className="h-16 px-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80">
            ...
            <button
              ...
              className={`hidden lg:flex p-1.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-opacity duration-150 flex-shrink-0 ${
                isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'
              }`}
            >
            ...
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center h-11 px-2.5 rounded-xl text-sm font-medium transition-colors duration-150 relative ${
                      isActive
                ...
                >
                  {({ isActive }) => (
                    <>
                      ...
                      {/* Text Label */}
                      <span
                        className={`ml-3 whitespace-nowrap text-xs font-semibold tracking-tight transition-opacity duration-150 ${
                          isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'
                        }`}
                      >
                        {item.name}
                      ...
                      {isActive && (
                        <div
                          className={`ml-auto pr-1 transition-opacity duration-150 ${
                            isCollapsed ? 'opacity-0' : 'opacity-60'
                          }`}
                        >
          ...
          <div className="p-3 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/40">
            <div className="flex items-center h-12 px-2 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700/60 shadow-sm">
              ...
              <button
                ...
                title="Sign Out"
                className={`p-1.5 rounded-lg text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/50 transition-opacity duration-150 flex-shrink-0 ${
                  isCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'
                }`}
              >
